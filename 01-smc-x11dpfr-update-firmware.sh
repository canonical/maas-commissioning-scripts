...
# name: 01-smc-x11dpfr-update-firmware
# title: SMC X11DPFR-S(N) Update   
# description: Update BMC and BIOS on X11DPFR-S and X11DPFR-SN motherboards
# type: commissioning
...
# destructive: False
# hardware_type: node
# for_hardware: mainboard_product:X11DPFR-S,mainboard_product:X11DPFR-SN 
# may_reboot: True
...
export SMC_MOBO=X11DPFR

# Directory to extract files/utils
export SMC_DIR='/opt/smc'

# Firmware URL Update the URLs below to keep script current
export SMC_FW_URL='https://www.supermicro.com/Bios/softfiles/11659/X11DPFR-S_3_3_AS17115_SUM240.zip	'
...
eval \
	$(awk '{ if (/^SMT.*AST/) print "export SMC_BMC_ZIP=\"opt/smc/"$1"\""; if (/^sum/) print "export SMC_SUM_ZIP=\"/opt/smc/"$1"\"";if (/^'${SMC_MOBO}'/) print "export SMC_BIOS_ZIP=\"/opt/smc/"$1"\""}' \
...
export SMC_BMC_IMAGE=$(find ${SMC_BMC_DIR} -type f -regextype "posix-extended" -iregex ".*(SMT_${SMC_MOBO:0:3}AST.*\.bin)")
